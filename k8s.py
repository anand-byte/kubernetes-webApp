...
# Launch Pod
    ...
    if 'pod' in str:
        ...
        i = str.index('pods') + 1
    j = str.index('image') + 1
    cmd = "sudo kubectl run {0} --image={1} --kubeconfig admin.conf".format(str[i], str[j])
    output = subprocess.getoutput(cmd)
    print("<pre>{}</pre>".format(output))
...
# Launch Deployment
    ...
    j = str.index('image') + 1
    cmd = "sudo kubectl create deployment {0} --image={1} --kubeconfig admin.conf".format(str[i], str[j])
    output = subprocess.getoutput(cmd)
    print("<pre>{}</pre>".format(output))
